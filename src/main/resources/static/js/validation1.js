...
var gen = document.getElementById("gender_id");
var geni = document.getElementById("gender_i");
var genv = document.getElementById("gender_v");
...
/*	var x = 0,i;
	...
	var letters = /^[A-Za-z]+$/;
	if(gen.value.match(letters)){

		if(gen.value=="male" || gen.value=="MALE" || gen.value=="Male" || gen.value=="female" || gen.value=="FEMALE" || 
			gen.value=="Female"){
		geni.innerHTML = "<i class='glyphicon glyphicon-ok'></i>";
		genv.innerHTML = "";	
		return true;

		}else{
		geni.innerHTML = "<i class='glyphicon glyphicon-remove'></i>";
		genv.innerHTML = "<span style='color:red'>invalid gender</span>";
		return false;	
		}	

	}else if(gen.value == ""){
		geni.innerHTML = "<i class='glyphicon glyphicon-remove'></i>";
		genv.innerHTML = "<span style='color:red'>enter gender</span>"
		return false;
	}

		geni.innerHTML = "<i class='glyphicon glyphicon-remove'></i>";
		genv.innerHTML = "<span style='color:red'>only alphabets</span>"
		return false;
...
/*	if(branch.value == "default"){
	...
	if(branch.value.match(letters)){
		...
		branchi.innerHTML = "<i class='glyphicon glyphicon-remove'></i>";
		branchv.innerHTML = "<span style='color:red'>enter branch</span>"
		return false;

	}

		branchi.innerHTML = "<i class='glyphicon glyphicon-remove'></i>";
		branchv.innerHTML = "<span style='color:red'>only alphabets</span>"
		return false;
...
function valid_pic(){
	...
	if(pic_format != ".jpg" && pic_format!=".png" && pic_format!=".JPG"){
		pici.innerHTML = "<i class='glyphicon glyphicon-remove'></i>";
		picv.innerHTML = "<span style='color:red'>upload in jpg format</span>"
		return false;
	}
...
function valid(){
	...
	var fname,lname,gender,eid,unit,des,contact,branch,add,city,pin,pic,email,dob;
	fname = valid_fname();
	lname = valid_lname();
	gender = valid_gender();
	eid = valid_eid();
	...
	if(fname==true && lname==true && gender==true && eid==true &&
		...
		add==true && city==true && pin==true  && email==true && dob==true && pic==true)
	{
